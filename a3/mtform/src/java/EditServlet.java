    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if ("Password=".equals(password) || "Verify=".equals(password2) || "First Name=".equals(fName)
                ...
                String tmp = x[0].substring(x[0].lastIndexOf("=") + 1);
                String tmp2 = x[1].substring(x[1].lastIndexOf("=") + 1);

                String tmp3 = x[4].substring(x[4].lastIndexOf("=") + 1);
                int ntmp3 = Integer.parseInt((tmp3).substring(0, tmp3.indexOf("-")));

                String tmp5 = x[2].substring(x[2].lastIndexOf("=") + 1);
                String tmp6 = x[3].substring(x[3].lastIndexOf("=") + 1);
                String tmp7 = x[7].substring(x[7].lastIndexOf("=") + 1);

                String tmp10 = x[6].substring(x[6].lastIndexOf("=") + 1);
                String tmp11 = x[5].substring(x[5].lastIndexOf("=") + 1);
                String tmp12 = x[8].substring(x[8].lastIndexOf("=") + 1);
